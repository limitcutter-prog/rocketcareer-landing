...
## 2026-07-02 21:07 — 멘토링 S3 doc_type SQL 적용·검증 (item10 완전 활성)

**무엇을**
- 사장이 `supabase-mentoring-doctype.sql`(case_journeys.doc_type) 적용 → 멘토 S2 '문서유형 지정' 명시 저장 활성.
- service-role 스크립트로 검증: doc_type 컬럼 SELECT OK(여정 5건) + 쓰기 라운드트립(resume 저장→원복) 정상. 검증 스크립트는 삭제.

**왜**
- 전날 배포(`edbeb45`)한 멘티 입력개선 item10(S3 문서유형)은 SQL 적용 시 멘토 명시 지정이 켜지는 구조 — 마지막 ⬜(사장 SQL) 해소.

**영향 / 후속**
- item10 완전 활성. 기존 여정 doc_type=null → 트랙 기본값(경력=경력기술서·신입=자소서), 멘토 지정 시 우선. 코드 커밋 없음(스키마·운영 이벤트).
- ⬜ 남은 멘토링 SQL: `mentor-onboarding`·`mentee-consent` 2종(별개 기능).

**커밋**: (코드 커밋 없음 — SQL 적용·검증) / 문서: `landing`(이 sync)
