# 작업 로그 아카이브 (WORKLOG ARCHIVE)

> WORKLOG.md에서 밀려난 과거 작업 블록 보관소. 최신이 맨 위.
> /sync-arch가 WORKLOG.md에 새 블록을 추가하면서 오래된 블록(최근 5개 초과분)을 이 파일 맨 위로 이동한다.
> 평상시 sync에서 이 파일을 읽지 말 것(토큰 절약). 과거 작업 추적 시에만 부분 Read.
