...
## 2026-06-26 (**멘토 셀프 어필 지면 + 매칭검토(사장 승인)** — 쇼케이스에서 멘토가 스스로를 파는 지면이 약하고(셀프편집=소개·태그·사진URL+아티클뿐, 노출=균일 카드+모달), 멘티 신청을 멘토가 직접 수락/거절해 사장이 매칭 품질을 통제 못 함. **Pillar A(멘토 지면)**: 신규 `mentor_blocks`(하이브리드 블록 — heading·rich·image·gallery·stat·quote·qna·video·cta, jsonb content·sort_order·is_visible) + `mentor_profiles` ALTER(cover_url·tagline·theme·accent_color) + 공개 버킷 `mentor-assets`. API `/api/mentor/blocks`(+`[id]` 소유권검증)·`/api/mentor/upload`(이미지 5MB·MIME)·`/api/mentor/me` 확장·신규 공개 `/api/showcase/mentors/[id]`(블록+서비스+후기+아티클, `?preview=1`)·목록 라우트 신규필드. 페이지 신규 `app/showcase/mentors/[id]`(에디토리얼 풀페이지·scroll-reveal) + `MentorBlocks.tsx` 렌더러 + 메인 `MentorSpotlight`(피처드+스크롤러, 카드→풀페이지 라우팅, 모달 폐지) + 멘토 포털 지면 에디터(커버/태그라인/테마/블록 추가·순서·공개·삭제·미리보기). **Pillar B(매칭검토)**: 신청 `mentee_applications.status='review'`(was submitted) → 신규 `/admin/matches`(OWNER_ONLY)·`/api/matches`(GET)·`[id]`(PATCH approve/reject) → 승인 시 matches(accepted)+match_events(by:owner)+status=matched+멘티·멘토 메일(Resend `sendMail`/`mailLayout`). 멘토 직접 수락/거절 폐지(`/api/mentor/requests/[id]` PATCH→403, 포털은 확인만). 권한 `lib/permissions` OWNER_ONLY +`/admin/matches`·`/api/matches`, 네비 '접수·멘티'에 매칭검토. 검증: tsc 0·API 신규필드·라우트 가드(401/307/200)·풀페이지 DOM 렌더. 🔒 FROZEN 무접촉(마켓플레이스 테이블만). 결제/정산은 범위 외)
